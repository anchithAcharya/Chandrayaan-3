...
SimpleObj_t* loadObj(const char* file_name);
...
static SimpleObj_t* newSimpleObj(void) {
  SimpleObj_t* obj = (SimpleObj_t*) malloc(sizeof (SimpleObj_t));
...
*/
SimpleObj_t* loadObj(const char* fileName) {
    ...
    if (strcmp(lineType, OBJ_GROUP) == 0
      ...
      if (peekEof) {
        ...
        ObjGroup_t* lastGroup
          = (ObjGroup_t*) objDataArrayAccess(&obj->groups, obj->groups.nextIndex-1);
        lastGroup->endFace++;
      }
    }
    #ifdef DEBUG
...
void disposeObj(SimpleObj_t* obj) {
  ...
  for (g = 0; g < obj->groups.nextIndex; g++) {
    ObjGroup_t* group = (ObjGroup_t*) objDataArrayAccess(&obj->groups, g);
    free(group->name);
  }
...
void drawObj(SimpleObj_t* obj) {
  ...
  for (g = 0; g < obj->groups.nextIndex; g++) {
    ...
    for (f = group->startFace; f < group->endFace; f++) {
      ObjDataArray_t* face = (ObjDataArray_t*) objDataArrayAccess(&obj->faces, f);
      glBegin(GL_POLYGON);
      int f_c;
      for (f_c = 0; f_c < face->nextIndex; f_c++) {
        ObjFaceComponent_t* faceComp = (ObjFaceComponent_t*) objDataArrayAccess(face, f_c);
        ObjVertex_t* vertex
          = (ObjVertex_t*) objDataArrayAccess(&obj->vertices, faceComp->vertexIndex-1);
        if (faceComp->normalIndex > 0) {
          ObjVertexNormal_t* normal
            = (ObjVertexNormal_t*) objDataArrayAccess(&obj->normals, faceComp->normalIndex-1);
          glNormal3d(normal->x, normal->y, normal->z);
        }
        if (faceComp->texCoordIndex > 0) {
          ObjTexCoord_t* texCoord
            = (ObjTexCoord_t*) objDataArrayAccess(&obj->texCoords, faceComp->texCoordIndex-1);
